...
| 接口名称 | 云端支付接口 |
| ------- | --------------- |
| 接口URL | https://gatepay.io/api/cloudpay/create |
| 请求方式 | POST |
| 返回格式 | JSON |
...
| order_id | integer | 是 | 订单编号 |
| out_order_id | string | 是 | 外部订单编号 |
| sign | string | 是 | 签名信息算法：sign = md5(md5(appkey + price + type + prodcut_id + order_id + out_order_id) + appsecret) |
